...
var batchFileAlreadyRunning = false;
...
const server = http.createServer(function (req, res) {
    ...
    switch (req.url) {
        case '/':
            ...
            break;
        case '/getFile':
            if(batchFileAlreadyRunning) {
                const msg = "Batch file " + batchFilePath + " already running, not running another one.";
                console.log(msg);
                res.writeHead(102); //102 = processing
                res.end(msg);
            } else {
                batchFileAlreadyRunning = true;
                //run bat file: 
